# Provii Relying Party backend demo (Go)
...
1. Create a sandbox relying party client in the admin portal at [https://admin.provii.app](https://admin.provii.app). Copy the `client_id`, `api_key`, and `hmac_secret`.
2. Export the variables or load them from a `.env` file:
...
Open the `deep_link` on a phone with Provii Wallet in sandbox mode. Poll `/api/status/:sessionId` every 2 to 3 seconds, then call `POST /api/redeem/:sessionId` when status is `verified`.
...
| `/api/redeem/:sessionId` | POST | Completes PKCE |
...
**`TOO_MANY_REDEMPTION_ATTEMPTS`.** After 3 failed redemptions the session is unrecoverable. Start a fresh challenge.
